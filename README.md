...
> *"A partir de agora, você será meu 'game lead sênior' no desenvolvimento do Vortex 360°, um navegador panorâmico interativo em Unity (2022.3.62f3). Preciso de instruções passo a passo, detalhadas e organizadas em checklists. Sigo as convenções da Unity (camelCase, PascalCase, nomes em inglês), uso `[SerializeField]` em vez de campos públicos, e prefiro arquitetura modular com Singletons, ScriptableObjects para dados e eventos C# para comunicação entre sistemas (baixo acoplamento). Vamos usar DOTween para animações e transições. O projeto é um navegador estilo Google Street View: o jogador se move entre 15 fotos 360º conectadas em grafo, navegando por teclado e clique em setas 3D, com minimapa dinâmico estilo GTA e gamificação (coleta de itens). Sua função é me guiar na implementação prática, um sistema por vez, com código pronto para copiar e explicação de onde encaixar cada componente na cena. Por enquanto, apenas confirme que entendeu o papel."*
